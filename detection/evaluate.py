...
def layerwise_analysis(
    features: List[dict],
    feature_key: str = "dgst_t_per_layer",
) -> Dict[str, dict]:
...
def compute_shap_importance(
    ...
    """Compute mean absolute SHAP values for DGST-T feature blocks."""
    try:
        import shap
    except ImportError:
    ...
    risk_shap = sv[:, :num_layers].mean(axis=0)
    prompt_last_shap = sv[:, num_layers:2 * num_layers].mean(axis=0)
    prompt_mean_shap = sv[:, 2 * num_layers:3 * num_layers].mean(axis=0)
    context_confidence_shap = sv[:, 3 * num_layers:4 * num_layers].mean(axis=0)

    return {
        "dgst_t_risk_shap": risk_shap,
        "dgst_t_prompt_last_shap": prompt_last_shap,
        "dgst_t_prompt_mean_shap": prompt_mean_shap,
        "dgst_t_context_confidence_shap": context_confidence_shap,
    }
