...
def build_strict_82_split(
    image_ids: Iterable[int],
    *,
    seed: int = 42,
) -> dict[str, list[int]]:
    """Build a deterministic outer 80/20 image split.

    ``val`` is retained as an empty compatibility field because the existing
    artifact schema uses train/val/test keys.  The strict protocol uses fixed
    training settings, the last epoch and a train-F1 threshold; test images are never
    used for checkpoint, threshold, or hyperparameter selection.
    """
    ...
    if len(unique_ids) < 2:
        raise ValueError("At least two unique image IDs are required")

    rng = random.Random(int(seed))
    rng.shuffle(unique_ids)
    n_train = int(len(unique_ids) * 0.8)
    if n_train <= 0 or n_train >= len(unique_ids):
        raise ValueError(
            f"80/20 split would contain an empty partition for {len(unique_ids)} IDs"
        )
    return {
        "train": unique_ids[:n_train],
        "val": [],
        "test": unique_ids[n_train:],
    }


def validate_strict_82_split(
    splits: Mapping[str, Sequence[int]],
    *,
    expected_image_ids: Optional[Iterable[int]] = None,
) -> dict[str, int]:
    """Validate a strict, leak-free outer 80/20 image split."""
    ...
    if normalized["val"]:
        raise ValueError(
            "strict 80/20 outer split requires an empty top-level val list; "
            "this protocol does not use a validation split"
        )
    train_ids = set(normalized["train"])
    test_ids = set(normalized["test"])
    overlap = train_ids & test_ids
    if overlap:
        raise ValueError(f"image train/test splits overlap: {len(overlap)}")
    ...
    expected_counts["test"] = count - expected_counts["train"]
    actual_counts = {name: len(normalized[name]) for name in SPLIT_NAMES}
    if actual_counts != expected_counts:
        raise ValueError(
            "image split is not strict 80/20: "
...
def ensure_strict_82_split(
    ...
    *,
    ...
    """Atomically install the canonical outer 80/20 split."""

    ids = [int(image_id) for image_id in image_ids]
    canonical = build_strict_82_split(ids, seed=seed)
    validate_strict_82_split(canonical, expected_image_ids=ids)
    ...
    if shared_splits_path is not None:
        ...
        _install_82_if_needed(shared_path, canonical, ids)
        canonical = _load_json(shared_path)

    output_path = Path(splits_path)
    backup_path = _install_82_if_needed(output_path, canonical, ids)
    installed = _load_json(output_path)
    validate_strict_82_split(installed, expected_image_ids=ids)
    return installed, backup_path


def _install_82_if_needed(
    path: Path,
    ...
    if path.exists():
        ...
        if _same_split(current, canonical):
            validate_strict_82_split(current, expected_image_ids=expected_ids)
            return None
        backup_path = _next_backup_path(path, protocol="82")
...
def _next_backup_path(path: Path, *, protocol: str = "811") -> Path:
    candidate = path.with_name(
        f"{path.stem}.pre_{protocol}{path.suffix}.bak"
    )
    index = 1
    while candidate.exists():
        candidate = path.with_name(
            f"{path.stem}.pre_{protocol}.{index}{path.suffix}.bak"
        )
        index += 1
    return candidate
