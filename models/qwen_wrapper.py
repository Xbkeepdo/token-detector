...
class QwenVLWrapper(BaseLVLMWrapper):
    ...
    def extract_token_features_batch(
        ...
        The generic extractor still calls the batch API for every wrapper.  Qwen's
        true batch path materializes attentions/captures for the full generated
        response and can OOM on COCO captions with many object tokens, so this
        override intentionally keeps the same interface while doing sequential
    ...
    def extract_prompt_target_features(
        ...
        """Extract the row predicting an object token inside the real prompt."""
        messages = [{
            "role": "user",
            "content": [
                {"type": "image", "image": image},
